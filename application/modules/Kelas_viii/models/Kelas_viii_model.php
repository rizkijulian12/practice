...
class Kelas_viii_model extends CI_Model {

	public function get_current_page($limit, $start) {
        $this->db->limit($limit, $start);
        $query = $this->db->get('kelas_8');
        $rows = $query->result();
	...
	{
    	$this->db->insert('kelas_8',$data);
	}

	public function ubah($data,$id)
	{
		$this->db->where('id' , $id);
		$this->db->update('kelas_8' , $data);
	}
