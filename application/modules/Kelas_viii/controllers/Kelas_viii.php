...
class Kelas_viii extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('Kelas_viii_model', 'Kelas_viii_model');
	...
	//set records per page
				...
				$total = $this->Kelas_viii_model->get_total();

				if ($total > 0)
				{
						// get current page records

						$config['base_url'] = base_url() . 'kelas_viii/index';
	 ...
	 $config['num_tag_close'] = '</li>';
				...
				$params['record'] = $this->Kelas_viii_model->get_current_page_bulan($limit_page, $page * $limit_page);
				$params['content'] ='kelas_viii/tampilan_bulan';
				$this->load->view('template',$params);
	}
	public function tampilan($bulan=null, $tahun=null)
{
	$data['content'] ='kelas_viii/tampilan';
	$data['count'] = $this->Kelas_viii_model->get_data_count($bulan, $tahun);
	$data['record'] = $this->Kelas_viii_model->get_data($bulan, $tahun);
	...
	$start_index = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
		 $total_records = $this->Kelas_viii_model->get_total();
	...
	$data['content']  = 'Kelas_viii/tampilan'; // load halaman pada views;
	...
	$data['link']   = 'Kelas_viii/tambah'; // variabel link untuk menambahkan data pada button tambah data jika tidak digunakan tambahkan FALSE;
	$data["record"]   = $this->Kelas_viii_model->get_record($limit_per_page, $start_index);
	...
	{
		$data['content'] ='Kelas_viii/form_tambah';
			$this->load->view('template',$data);
	}
	public function ubah($id=null)
	{
		$data['content'] ='Kelas_viii/form_ubah';
		$data['record'] = $this->Kelas_viii_model->get_data_kelas_viii($id);
	...
	public function proses_tambah()
	{
			$data = array(
				...
				'tahun' => $this->input->post('tahun')
						...
						$insert = $this->Kelas_viii_model->insert_data($data);
		//helper_log("add", "Menambah Daftar Pengguna");

			redirect ('Kelas_viii/tampilan/'.$this->uri->segment(3).'/'.$this->uri->segment(4));
			//helper_log("add", "Menambah Daftar Pengguna");
	}
		...
		public function proses_ubah($id)
	{
			...
			//'gambar' => $this->input->post('filex'),
			 ...
			 $this->Kelas_viii_model->ubah($data, $id);
			 redirect('Kelas_viii/tampilan/'.$this->uri->segment(4).'/'.$this->uri->segment(5));
				}
		//helper_log("add", "Menambah Daftar Pengguna");
		...
		public function proses_hapus($id=null)
	{
		...
		$delete = $this->Kelas_viii_model->delete_data($id);
		redirect('Kelas_viii/tampilan/'.$this->uri->segment(4).'/'.$this->uri->segment(5));
	}
}
